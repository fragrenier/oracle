...
ruby_block 'set_client_version_attr' do
  block do
    patch_info = %x(sudo -u #{node[:oracle][:cliuser][:edb]} #{node[:oracle][:client][:ora_home]}/OPatch/opatch lsinventory -bugs_fixed | grep -E '(#{node[:oracle][:rdbms][:latest_patch][:dirname]}) {3}\\1.* DATABASE PATCH SET UPDATE')
